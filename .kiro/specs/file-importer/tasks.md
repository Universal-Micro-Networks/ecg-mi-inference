...
- [x] `mfer_tools.extract_mfer_header()` を利用したヘッダ抽出を実装
  - [x] `MWF_TIM` から検査日時を抽出（XML より優先）
  - [x] `MWF_PID` / `MWF_PNM` を患者 ID・氏名にマッピング（XML より優先）
  - [x] `MWF_SEX` を性別にマッピング（XML より優先、正規化ルールは設計書）
  - [x] `MWF_AGE` を年齢にマッピング（生年月日形式・整数年齢の解釈、新規患者 `Patient.age` に反映）
- [x] XMLフォールバックパーサを実装
...
- [x] 単体テスト
  - [x] 拡張子判定（`.mwf/.MWF`）
  - [x] ヘッダ抽出マッピング（`MWF_*` 五キー・性別正規化・`MWF_AGE` パース）
  - [x] XMLフォールバック抽出
- [x] 統合テスト
