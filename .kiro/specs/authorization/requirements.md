...
7. If 管理者パスワードが未設定の場合, then 認可機能 shall ログイン API で HTTP 403 を返し初回セットアップを促す

### 要件1.1: 初回パスワードセットアップ（Bootstrap）

**目的:** システム管理者として、初回起動時に管理者パスワードをUIから設定したい。  
これにより、環境変数で初期パスワードを配布せずに安全に初期化できる。

#### 受け入れ基準

1. When システムに管理者パスワードが未設定の場合, 認可機能 shall 初回設定が必要な状態を返す（bootstrap status）
2. When 未設定状態で初回パスワードを送信する, 認可機能 shall パスワードポリシーを検証してDBへ保存する
3. If 既に管理者パスワードが設定済みの場合, then 認可機能 shall bootstrap API を HTTP 403 で拒否する
4. The 認可機能 shall 初回パスワードを環境変数に依存しない
...
8. When ユーザー操作が一定時間（既定8時間）ない場合, 認可機能 shall ローカルトークンを削除してログイン画面へ遷移する
