# 要件定義書: 認可・認可機能 (authorization)

## はじめに

本ドキュメントは、ECG心筋梗塞リスク推論システムにおける認可・認可機能の要件を定義する。
...
3. When ユーザーが正しいユーザー名とパスワードを入力して送信する, 認可機能 shall 認可トークン（JWT）を発行しセッションを開始する
...
| 用語 | 定義 |
|------|------|
| 認可（Authentication） | ユーザーの身元を確認するプロセス |
| 認可（Authorization） | 認可済みユーザーに対するアクセス権限の制御 |
