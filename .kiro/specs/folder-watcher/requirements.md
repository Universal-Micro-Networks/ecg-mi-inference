...
6. `MFER_WATCH_FOLDER` shall 絶対パスを受け付ける（ローカルボリューム、macOS の `/Volumes/...`、Linux のマウントポイント、Windows のドライブレターおよび UNC `\\server\share\...` 等を含む）
7. 絶対パスの正規化（シンボリックリンク解決等）で OS エラーが発生した場合, then フォルダ監視機能 shall 警告ログを出力し、指定されたパス文字列に基づいて監視の開始を試みる
8. SMB / NFS 等のネットワーク共有上のフォルダを監視する場合に備え、環境変数により Watchdog の `PollingObserver`（定期ディレクトリ走査）を選択できること shall とする（OS ネイティブの inotify / FSEvents 等がイベントを落としやすい構成向け）
...
5. ヘルスチェックレスポンスには、ポーリング監視を使用しているか（`use_polling_observer` 等）を含め、運用者がネットワーク共有向け設定を確認できること shall とする
6. フォルダ監視機能 shall ヘルスチェックポートを環境変数 `MFER_HEALTH_PORT` で設定可能とする（デフォルト: 8081）
...
| 変数名 | 必須 | デフォルト | 説明 |
|--------|------|-----------|------|
| `MFER_WATCH_FOLDER` | ✓ | - | 監視対象フォルダのパス（相対または絶対。`~` 展開可） |
| `MFER_WATCH_USE_POLLING` | - | `false` | `true` 等で Watchdog `PollingObserver` を使用（ネットワーク共有推奨） |
| `MFER_WATCH_POLLING_INTERVAL_SEC` | - | `1` | ポーリング監視の間隔（秒） |
