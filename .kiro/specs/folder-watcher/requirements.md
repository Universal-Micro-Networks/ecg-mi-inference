...
本ドキュメントは、ECG心不全リスク推論システムにおけるフォルダ監視機能の要件を定義する。
...
- **file-importer**: MFERファイルから患者属性を抽出、患者・検査データのDB登録（検索用）、MFERファイルパスとの紐付け
- **ecg-mi-inferencer（別途定義）**: 心電図波形の読み込み、CSV変換、画像変換、心不全リスク推論

**関連ユースケース:** UC-1（システムは指定フォルダを監視し、新規MFERデータ追加時に検査データ・患者データをDBへ自動登録する）の「監視・検出」部分
