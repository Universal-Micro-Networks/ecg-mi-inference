# 要件定義書: 心不全リスク推論機能 (ecg-mi-inferencer)

## はじめに

本ドキュメントは、ECG心不全リスク推論システムにおける心不全リスク推論機能の要件を定義する。
本機能は、検査UUIDを受け取り、MFERファイルから心電図波形データを読み込み、Vision Transformerモデルを用いて
心不全リスクを推論するバックエンドサービスである。
...
**目的:** システムとして、Vision Transformerモデルを用いて心不全リスクを推論したい。
...
| リスクスコア | 0.0-100.0の数値で表される心不全リスクの程度 |
