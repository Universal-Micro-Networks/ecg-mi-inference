...
**責務境界:**
- **diagnosis-list（本機能）**: 診察データの一覧表示、フィルタリング（検査日/患者ID/氏名：バックエンド検索）、ソート、ページング、診察選択
...
4. 診察一覧機能 shall 総件数と表示範囲（例: 1〜50 件 / 全 120 件）を表示する
...
**サーバー検索（患者ID、患者氏名）:**
5. 診察一覧機能 shall 患者IDによる検索フィールドを提供する（入力値は確定前状態として保持）
6. 診察一覧機能 shall 患者氏名による部分一致検索フィールドを提供する（入力値は確定前状態として保持）
7. 診察一覧機能 shall 患者ID・患者氏名フィルターをAPIクエリに含めてバックエンド側で適用する
8. 診察一覧機能 shall 患者ID・患者氏名フィルターは **Enter/Return キー押下時にのみ** APIクエリへ反映する（IME 変換確定中の Enter は除外）
9. 診察一覧機能 shall フィルター条件のクリア専用ボタンを必須としない（利用者は入力欄を空にして Enter で検索解除できる）
...
3. When 行が選択される, 診察一覧機能 shall **別ページへは遷移せず**、画面右から **スライドパネル**を開き、その中で診察詳細（`ExaminationDetailView`）を表示する
4. 診察一覧機能 shall 選択中の診察 ID を URL クエリ **`detail`** に反映する（例: `/diagnoses?exam_date=…&detail=<UUID>`）。既存のフィルタ・ソート・ページング用クエリを維持する
5. 診察一覧機能 shall パネル幅を**ビューポート幅の約 2/3**とする（狭い画面では全幅表示に切り替えてよい）
6. 診察一覧機能 shall パネル外の**半透明バックドロップ**クリックおよび **Escape**（子ダイアログ未表示時）でパネルを閉じ、閉じアニメーション完了後に `detail` をクエリから除去する
7. 診察一覧機能 shall パネル表示中に `document.body` のスクロールを抑制してよい
8. 推論の実行・結果表示は**詳細パネル内の「判定」モーダル**で行う（一覧行に「推論実行」専用ボタンを置くことは必須としない）
...
1. 診察一覧機能 shall 認可付き `GET /api/examinations/events`（SSE）を購読し、`examinations_changed` 受信時に一覧クエリを再取得する
2. 診察一覧機能 shall SSE 接続が切断された場合に自動再接続を試行する
3. When 画面がフォーカス状態に戻る（タブ切り替え後等）, 診察一覧機能 shall 自動リフレッシュを実行する
4. 診察一覧機能 shall 最後の更新日時を表示する（例: 「最終更新: 2025-12-07 14:30」）
5. 診察一覧機能 shall 手動リフレッシュボタンを必須としない

### 要件6: ページング

**目的:** 医療従事者として、件数が多い日付でも段階的に一覧を確認したい。

#### 受け入れ基準

1. 診察一覧機能 shall `limit` / `offset` をAPIクエリとして送信する
2. 診察一覧機能 shall 前へ/次へ操作で `offset` を更新して再取得する
3. 診察一覧機能 shall 検査日・ソート・検索条件変更時に `offset` を 0 にリセットする
4. バックエンドAPI shall `items` と `total` を返却する
...
- 診察一覧機能 shall 患者ID・患者氏名フィルターをサーバーサイドで実行し、必要件数のみ取得する
...
| `patient_id` | String | - | 患者外部IDの部分一致フィルター |
| `patient_name` | String | - | 患者氏名の部分一致フィルター |
| `limit` | Integer | - | 取得件数（1〜500、デフォルト: 50） |
| `offset` | Integer | - | 取得開始位置（0以上、デフォルト: 0） |
...
| フィルタリング | 診察データを条件で絞り込む機能（検査日・患者外部ID・氏名：バックエンド、ページングあり） |

---

**ステータス:** 実装反映済み（診察詳細スライドパネル・`detail` クエリ）
**作成日:** 2025-12-07
**最終更新:** 2026-04-07（Enter確定検索・SSE更新の書き戻し）
